...
def get_time_range(num_days, timezone='UTC'):
    """Generate from and to time strings. To be used for fetching data for a specific number of days."""
...
def process_high_tides(data, timezone):
    """Process high tides data. To return a list of high tides."""
    high_tides = []
...
def process_forecasts(data, forecast_type):
    """Process weather and ocean forecasts. To return a list of forecasts."""
    forecasts = []
...
def read_apis(latitude, longitude, num_days):
    """Fetch and combine data from different sources. Such as tide, weather, and ocean forecasts.""" 
    ...
    sun_data = fetch_data(sunrise_api_url, params={"lat": latitude, "lon": longitude})
    print(sun_data)
...
latitude, longitude, num_days = config.get("latitude", 60), config.get("longitude", 5.0), config.get("NumDays", 30)

high_tides, weather_forecast, ocean_forecast = read_apis(latitude, longitude, num_days)
